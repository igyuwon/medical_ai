...
update students b set no=(select rnum from(select rownum rnum from students a))
where a.id = b.id
;
...
-- 동일한 컬럼은 중복이 없어야함. 2개중 하나 테이블에서는 primary 키가 사용이 되어야함.
...
-- stu_score, rank 순위를 update입력하시오.
select total, rank() over(order by total desc) ranks from stu_score
where no = no;
...
-- 기본 sql left outer join, right outer join 만 가능
