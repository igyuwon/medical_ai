...
insert into mem (no2) values(11111); -- 에러
insert into mem (no2) values(-9999);
...
select * from mem3;
commit;
select * from mem3 where name='aaa'; -- sql 구문은 대소문자 구분없음, 데이터는 대소문자 구분함.
...
select salary*12 as 달러기준연봉,salary*1381.79*12 as 원화기준연봉 from employees;

-- 커미션은 실제 월급 = 월급 + 월급*커미션
-- commission_pct
select * from employees;
