...
while True:
    ...
    if choice == 1:
        ...
        # 2-2. 못 찾았으면 다시 이전 화면으로 나옴
        print('학생 성적 수정입니다.')
        cnt = 0
        search = input('수정할 학생 이름을 입력하세요 : ')
        for i in students:
            if i.name.find(search) != -1:
                print(f'{i.name} 학생을 찾았습니다.')
                #subject_list = ['','국어','영어','수학']
                subject = int(input('수정할 과목을 입력하세요.(1. 국어, 2. 영어, 3. 수학, 0. 나가기) : '))
                if subject == 1:
                    i.kor = int(input('수정할 국어점수를 입력하세요. : '))
                elif subject == 2:
                    i.eng = int(input('수정할 영어점수를 입력하세요. : '))
                elif subject == 3:
                    i.math = int(input('수정할 수학점수를 입력하세요. : '))
                elif subject == 0:
                    break
            else:
                print('찾는 학생이없습니다.')
